...
def edit_fire(edited_folder: Path, extract_folder: Path, override_folder: Path):
    debug_log("editing fire pcfs...", DebugLogLevel.HUMAN)
    ...
    color1_should_be = [0, 235, 250, 255]
    color2_should_be = [254, 0, 251, 255]
    ...
    system_remover = PcfSystemSelector(
        [
            "incendiary"
            #"weapon_tracers_incendiary",
            #"weapon_tracers_2incendiary",
            #"weapon_tracers_3incendiary",
            #"weapon_tracers_4incendiary"
        ]
    )

    file_selector = PcfFileSelector(filename_exclude_keywords=exclude_files)
    pcf_editor = PcfEditor(patches, system_selector, file_selector, system_remover)
    ...
    debug_log("finished editing fire pcfs!", DebugLogLevel.HUMAN)
...
if __name__ == "__main__":
    if len(sys.argv) == 4:
        edit_fire(Path(sys.argv[1]), Path(sys.argv[2]), Path(sys.argv[3]))
    else:
