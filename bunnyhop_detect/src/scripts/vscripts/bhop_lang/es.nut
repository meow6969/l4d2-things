...
//// then add a single line `IncludeScript("path/to/your/language.nut")` 
//// i recommend putting your language file in the directory "scripts/vscripts/bhop_lang/" (without the starting "scripts/vscripts/")
////
...
::BhopLang.ES <-
{
	...
	Misc =
	{
		Introduction				= "hola %%NAME%%! pareces ser nuevo / nueva a MeowBhopDetect!\n"+
									  "escribe \"%%OLIVE_GREEN%%%%PREFIX%% help%%WHITE%%\" para ver la lista de comandos, y haz \"%%OLIVE_GREEN%%%%PREFIX%% toggle%%WHITE%%\" para deshabilitarme/habilitarme!",
	...
	BhopAnnounce = 
	{
		...
		Time						= "%%NAME%% bhopeo por %%DURATION%% seguidos!",
		FirstRecord					= "%%NAME%% obtuvo su primer record de bunnyhop!",
		BeatPB						= "%%NAME%% vencio su record de bunnyhop! %%SCORE_DIFFERENCE%% puntos!",
		BeatSessionPB				= "%%NAME%% vencio su record de bunnyhop de esta sesión! %%SCORE_DIFFERENCE%% puntos! "
	}
	...
	Commands =
	{
		...
		prefix =
		{
			...
			SuccessfullyChanged		= "prefijo de bhop detector cambiado desde %%PREFIX%%->%%PREFIX%%"
		}
