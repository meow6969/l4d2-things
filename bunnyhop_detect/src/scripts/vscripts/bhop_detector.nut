...
class ::BhopClasses.BhopChainData
{
	...
	function ScoreBhop()  // -> bool|int  (false means not best bhop, true means new best bhop, 69 means user first bhop)
	{
		...
		local pSID = ::BhopFunc.GetPlayerSteamID(this.player);
...
::BhopFunc <-
{
	...
	function EnsurePlayerSettings(player)
	{
		...
		if (!(pSID in ::BhopVars.PlayerSettings))
		{
			...
			::BhopVars.PlayerSettings[pSID] <- ::BhopClasses.PlayerSettings();
	...
	function UtilityTick()
	{
		...
		foreach (i, userid in ::BhopVars.PlayerInitList)
		{
			...
			if (r)
			{
				ClientPrint(player,3,"you have been initialized by bhop detect!");
...
::BhopEvent <-
{
	...
	{
		if (::BhopVars.PlayerInitList.find(params.userid) == null)
		{
